...
{      
    ...
    if (p->Par.PWMSwitchFrequency <= 0.0f)
    {
        p->Par.PWMSwitchFrequency = 10000.0f;
    }
    pwm_period = 1.0f / p->Par.PWMSwitchFrequency;

    for(i=0;i<7;i++)
    {
    	p->PwmOutTemp.State1[i]  = U_BLANK;
    	p->PwmOutTemp.Timer1[i]  = (unsigned int)(p->Par.FPGACountPerS * pwm_period / 7.0f);
    }
    p->Val.PwmVecterPeriod = pwm_period;
}
...
void  svpwm_calc(PWM *p)
{
    ...
    {
        float active_time_sum = p->Val.T1 + p->Val.T2;
        if (active_time_sum > 1.0e-9f)
        {
            float scale = p->Val.PwmVecterPeriod / active_time_sum;
            p->Val.T1 = p->Val.T1 * scale;
            p->Val.T2 = p->Val.T2 * scale;
            p->Val.T0 = 0.0f;
        }
        else
        {
            p->Val.T1 = 0.0f;
            p->Val.T2 = 0.0f;
            p->Val.T0 = p->Val.PwmVecterPeriod;
        }
    }
    if(p->Val.T1<0)
    p->Val.T1=0;
