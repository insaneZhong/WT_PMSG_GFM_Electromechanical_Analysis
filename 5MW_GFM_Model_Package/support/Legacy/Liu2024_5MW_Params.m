...
p.aero_rated_wind_speed = 11.487; % ideal rotor reference, before EMT losses
% The switching EMT plant needs about 5.68 MW mechanical input to export
% 5 MW.  Therefore the validated Region-3 control boundary is 12.2 m/s;
% using 11.487 m/s here incorrectly forces rated electrical power at 11.5.
p.rated_wind_speed = 12.20;
% Calibrated EMT mechanical input required for 5 MW PCC export.  The
% nonlinear switching plant includes PMSG, converter and filter losses;
% Task-3's 13 m/s tail gives Paero ~= 5.747 MW at Pgrid ~= 4.998 MW.
p.P_aero_rated_emt = 1.15*p.P_wt_rated;
p.rotor_radius = 63;
p.air_density = 1.225;
...
% Region 2.5 speed blend.  Keep 8/10 m/s on pure Kopt*omega^3 and begin
% blending only as rotor speed approaches/exceeds the rated-speed anchor.
% The speed blend is retained for startup and Region 2.5.  At above-rated
% wind, the nonlinear model subsequently performs a time-smoothed transfer
% to the fixed rated-power plateau; this avoids both a 5 MW command at zero
% speed and a persistent omega^3/pitch interaction at the final operating
% point.
p.mppt_rated_blend_low_pu = 0.90;
p.mppt_rated_blend_high_pu = 0.98;
p.rated_power_transition_start_s = 20.0;
p.rated_power_transition_duration_s = 10.0;
% The proportional speed term already supplies most of the approximately
% 0.523 deg pitch observed at 12.2 m/s; do not count that trim twice in the
% wind feedforward.
p.pitch_beta_rated_ff_deg = 0.0;
% Calibrated from the accepted 12.2 m/s point: Paero = 5.678 MW,
% beta = 0.523 deg.  Holding that mechanical input at 13 m/s requires
% beta = 1.147 deg, hence (1.147-0.523)/(13-12.2) ~= 0.78 deg/(m/s).
p.pitch_beta_ff_gain_deg_per_mps = 0.78;
p.pitch_ff_start_wind_speed = p.rated_wind_speed;
p.pitch_kp_deg_per_radps = 10.0;
% Keep the pitch loop deliberately slower than the electromechanical
% controls.  Rated-operation startup is shortened through a consistent
% integrator initial condition in the nonlinear parameter script instead
% of increasing this gain and creating a 60 s pitch-speed overshoot.
p.pitch_ki_deg_per_rad = 0.01;
% With the staged transfer to a fixed 5 MW rated plateau, the 13 m/s
% equilibrium lies at the zero-pitch boundary.  A positive initial bias
% produced a minute-scale pitch and rotor-speed drift after the electrical
% variables had already settled.
p.pitch_integrator_initial_13mps_deg = 0.0;
p.pitch_integrator_initial_13mps_deg_gfl = 0.135;
