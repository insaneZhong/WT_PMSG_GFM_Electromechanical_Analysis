function T = scan_steady_nondisturbance_timing_powerloop()
% Scan presynchronization switch time and active-power loop PI in no-disturbance mode.
% Goal: find combinations with better steady-state indicators near 1 MW.

root = fileparts(mfilename('fullpath'));
old = pwd; cleanup = onCleanup(@() cd(old)); cd(root);

origH = fileread(fullfile(root,'grid_forming_control.h'));
origC = fileread(fullfile(root,'grid_forming_control.c'));
restoreObj = onCleanup(@() restore_files(fullfile(root,'grid_forming_control.h'), fullfile(root,'grid_forming_control.c'), origH, origC));
...
rows = [];
for tSync = presynList
    for kp = kpList
        for ki = kiList
            patch_header(tSync, kp, ki);
            run_case = run_one_case(pref, simStop, vacRef, vdcRef, qref);
            rows = [rows; tSync, kp, ki, run_case]; %#ok<AGROW>
        end
    end
end
...
T = array2table(rows, 'VariableNames', { ...
    'tSync', 'kpp', 'kip', ...
    'Pref_end', 'Pmeas_mean', 'Ppcc_mean', 'Udc_mean', 'Udc_slope', ...
    'Wref_mean', 'OmegaG_slope', 'ThetaTw_slope'});

outDir = fullfile(root, 'Validation_Results');
if ~exist(outDir,'dir'), mkdir(outDir); end
writetable(T, fullfile(outDir, 'scan_steady_nondisturbance_timing_powerloop.csv'));
disp(T);
fprintf('Saved: %s\n', fullfile(outDir,'scan_steady_nondisturbance_timing_powerloop.csv'));
end

function patch_header(tSync, kp, ki)
pth = fullfile(fileparts(mfilename('fullpath')),'grid_forming_control.h');
txt = fileread(pth);
txt = regexprep(txt, '#define\s+PRESYN_SWITCH_TIME\s+[0-9\.]+', ...
    sprintf('#define   PRESYN_SWITCH_TIME                    %.6g', tSync), 'once');
txt = regexprep(txt, ['#define\s+POWER_LOOP_PI_DEFAULTS\s+\{0\.0,\s*0\.0,\s*0\.0,\s*' ...
    '[^,]+,\s*[^,]+,\s*0\.0,\s*0,'], ...
    sprintf('#define POWER_LOOP_PI_DEFAULTS   {0.0, 0.0, 0.0, %.8g, %.8g, 0.0, 0,', kp, ki), 'once');
fid = fopen(pth,'w'); fwrite(fid,txt); fclose(fid);
end

function m = run_one_case(pref, simStop, vacRef, vdcRef, qref)
% returns [Pref_end, Pmeas_mean, Ppcc_mean, Udc_mean, Udc_slope, Wref_mean, OmegaG_slope, ThetaTw_slope]
setenv('MW_MINGW64_LOC','C:\mingw64');
evalin('base','clear mex');
evalin('base','bdclose(''all'')');

% recompile after macro change
useSchemeA = false;
...
function restore_files(hPath,cPath,hTxt,cTxt)
fid=fopen(hPath,'w'); fwrite(fid,hTxt); fclose(fid);
fid=fopen(cPath,'w'); fwrite(fid,cTxt); fclose(fid);
end
