...
{	
    float voltage_ref_cmd = p->ref.voltage_ref;
...
//       } 
       if  (p->val.Pre_syn == 1 )
       {
             voltage_ref_cmd = p->ref.voltage_ref  + 3.45/100000.0 *(p->ref.Q_reactive_power_ref - p->val.pcc_Q_reactive_Power_filter );
             p->bak.CosPos     =  cos( p->pf.thet_ref );
 	         p->bak.SinPos     =  sin( p->pf.thet_ref );
       }

     p->pf.E_voltage_amplitude = voltage_ref_cmd;
