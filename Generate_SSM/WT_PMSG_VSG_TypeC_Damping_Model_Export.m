...
%% WT-PMSG GFM-MWT MSC-DVC Type c + APCAD Small-Signal Model
% Type c DVC is used as the engineering baseline; APCAD adds the shaft
% torsional damping command in the grid-side active-power reference.
...
syms wr mp w wn h k_p_mppt%Parameters
...
% 12) MSC outer DC-link voltage controller - Type c
% Type c adds grid-side active-power feedforward p_m to the Type-a DVC path.
syms vdc_ref v_dc gamma_dc %Variables
syms k_pdc k_idc k_ff_msc_typec %Parameters

x_DVC = gamma_dc;
u_DVC = [vdc_ref; v_dc; p_m];
y_DVC = [i_m_d_ref; i_m_q_ref];

f_DVC = vdc_ref - v_dc;
h_DVC = [0;
         k_pdc*(vdc_ref - v_dc) + k_idc*gamma_dc + k_ff_msc_typec*p_m];

J_fx = jacobian(f_DVC, x_DVC); J_fu = jacobian(f_DVC, u_DVC);
J_hx = jacobian(h_DVC, x_DVC); J_hu = jacobian(h_DVC, u_DVC);
...
% 14) Shaft torsional damping controller
% The dominant electromechanical torsional mode identified by eigenvalue
% analysis is close to 2 Hz. This controller extracts the generator-speed
% component around that frequency with a second-order band-pass filter,
% compensates the phase lag with a lead element, and injects the resulting
% damping signal into the VSG active-power reference channel.
% Inputs: generator speed perturbation omega_g.
% Output: auxiliary active-power damping command p_damp.
syms x_bp1 x_bp2 x_lead p_damp %Variables
syms K_damp w_damp zeta_damp T_lead_damp alpha_lead_damp %Parameters

x_DAMP = [x_bp1; x_bp2; x_lead];
u_DAMP = omega_g;
y_DAMP = p_damp;

f_DAMP = [
    x_bp2;
    -w_damp^2*x_bp1 - 2*zeta_damp*w_damp*x_bp2 + 2*zeta_damp*w_damp*omega_g;
    -(1/(alpha_lead_damp*T_lead_damp))*x_lead + x_bp2];

h_DAMP = K_damp*((alpha_lead_damp - 1)/(alpha_lead_damp^2*T_lead_damp)*x_lead + (1/alpha_lead_damp)*x_bp2);

J_fx = jacobian(f_DAMP, x_DAMP); J_fu = jacobian(f_DAMP, u_DAMP);
J_hx = jacobian(h_DAMP, x_DAMP); J_hu = jacobian(h_DAMP, u_DAMP);
...
A_d0 = blkdiag(A_VSG, A_RPC, A_VC, A_CC, A_Del, A_LCL, A_Grid, A_TMS, A_PMSG, A_MSC, A_DVC, A_DC, A_DAMP);
B_d0 = blkdiag(B_VSG, B_RPC, B_VC, B_CC, B_Del, B_LCL, B_Grid, B_TMS, B_PMSG, B_MSC, B_DVC, B_DC, B_DAMP);
C_d0 = blkdiag(C_VSG, C_RPC, C_VC, C_CC, C_Del, C_LCL, C_Grid, C_TMS, C_PMSG, C_MSC, C_DVC, C_DC, C_DAMP);
D_d0 = blkdiag(D_VSG, D_RPC, D_VC, D_CC, D_Del, D_LCL, D_Grid, D_TMS, D_PMSG, D_MSC, D_DVC, D_DC, D_DAMP);

X_stac = [x_VSG; x_RPC; x_VC; x_CC; x_Del; x_LCL; x_Grid; x_TMS; x_PMSG; x_MSC; x_DVC; x_DC; x_DAMP];
U_stac = [u_VSG; u_RPC; u_VC; u_CC; u_Del; u_LCL; u_Grid; u_TMS; u_PMSG; u_MSC; u_DVC; u_DC; u_DAMP];
Y_stac = [y_VSG; y_RPC; y_VC; y_CC; y_Del; y_LCL; y_Grid; y_TMS; y_PMSG; y_MSC; y_DVC; y_DC; y_DAMP];

ustac_eq = [% Input to Active Controller
            p_ref + k_p_mppt*omega_g + p_damp; % MPPT slope plus shaft damping command
            ...
            i_m_q;
            omega_g;
            % Input to MSC DC-link voltage controller
            vdc_ref;
            v_dc;
            p_m_eq;
            ...
            p_msc;
            p_m_eq;
            % Input to shaft torsional damping controller
            omega_g;
            ];

U_sys = [p_ref; q_ref; Vg_D; Vg_Q; T_m; v_w; beta; vdc_ref];
y_sys_eq = [i1_d; i1_q; Vc_d; Vc_q; p_m_eq; q_m_eq; omega_t; omega_g; theta_tw; T_sh; T_aero; T_e; p_msc; v_dc; i_m_d; i_m_q; p_damp]; 
Y_sys = [i1_d; i1_q; Vc_d; Vc_q; p_m; q_m; omega_t; omega_g; theta_tw; T_sh; T_aero; T_e; p_msc; v_dc; i_m_d; i_m_q; p_damp];
...
VSG.dvc_type = 'MSC-DVC Type c + APCAD';
VSG.dvc_description = 'Delta i_m_q_ref = G_dvc(s)*Delta v_dc + K_ff*Delta p_m, with APCAD in active-power reference';

Unified_GFMI = struct(VSG);
export_dir = fullfile(fileparts(mfilename('fullpath')), 'Generated_Models');
if ~exist(export_dir, 'dir')
    mkdir(export_dir);
end
save(fullfile(export_dir, 'Unified_WT_PMSG_VSG_TypeC_Damping.mat'), 'Unified_GFMI');
