...
%% WT-PMSG GFM-MWT MSC-DVC Type a Small-Signal Model
% Type a: DC-link voltage PI directly generates the MSC q-axis current
% reference. This is the clean mechanism baseline:
%   Delta i_m_q_ref = G_dvc(s) * Delta v_dc
...
% 12) MSC outer DC-link voltage controller - Type a
% In Type a, the MSC-DVC uses only the DC-link voltage error. It does not
% feed forward grid-side output power into the machine-side current command.
...
A_d0 = blkdiag(A_VSG, A_RPC, A_VC, A_CC, A_Del, A_LCL, A_Grid, A_TMS, A_PMSG, A_MSC, A_DVC, A_DC);
B_d0 = blkdiag(B_VSG, B_RPC, B_VC, B_CC, B_Del, B_LCL, B_Grid, B_TMS, B_PMSG, B_MSC, B_DVC, B_DC);
C_d0 = blkdiag(C_VSG, C_RPC, C_VC, C_CC, C_Del, C_LCL, C_Grid, C_TMS, C_PMSG, C_MSC, C_DVC, C_DC);
D_d0 = blkdiag(D_VSG, D_RPC, D_VC, D_CC, D_Del, D_LCL, D_Grid, D_TMS, D_PMSG, D_MSC, D_DVC, D_DC);

X_stac = [x_VSG; x_RPC; x_VC; x_CC; x_Del; x_LCL; x_Grid; x_TMS; x_PMSG; x_MSC; x_DVC; x_DC];
U_stac = [u_VSG; u_RPC; u_VC; u_CC; u_Del; u_LCL; u_Grid; u_TMS; u_PMSG; u_MSC; u_DVC; u_DC];
Y_stac = [y_VSG; y_RPC; y_VC; y_CC; y_Del; y_LCL; y_Grid; y_TMS; y_PMSG; y_MSC; y_DVC; y_DC];

ustac_eq = [% Input to Active Controller
            p_ref + k_p_mppt*omega_g; % MPPT active-power reference slope for GFM-MWT
...
VSG.dvc_type = 'MSC-DVC Type a';
VSG.dvc_description = 'Delta i_m_q_ref = G_dvc(s)*Delta v_dc';
...
save(fullfile(export_dir, 'Unified_WT_PMSG_VSG_TypeA.mat'), 'Unified_GFMI');
